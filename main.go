...
func main() {
  ...
  // y := []float64{0.0,1.0,2.0,3.0}
  // offsetX := int(math.Max(0, float64(len(x)-len(y))))
  // offsetY := int(math.Max(0, float64(len(y)-len(x))))
  // fmt.Println(y[offsetY:], x[offsetX:])
  //fmt.Println(math.Max(0, -1))
}
