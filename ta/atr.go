...
func Atr(values []HighLowClose, period int) []float64 {
  // var atrs []float64
  // hlc := values[0]
  // tr := hlc.High-hlc.Low // since we have no previous entry, this is the True Range for the first period
  // atr := tr * float64(1/period)
  ...
  //   atr = ((atrs[idx+period-1] * float64(period-1)) + tr) / float64(period)
  //   atrs = append(atrs, atr)
  // }
  // return atrs
